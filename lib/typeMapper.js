...
function toGraphQL(sequelizeType, sequelizeTypes) {
  ...
  if (sequelizeType instanceof STRING || sequelizeType instanceof TEXT || sequelizeType instanceof UUID || sequelizeType instanceof DATE || sequelizeType instanceof DATEONLY) {
    ...
    return new _graphql.GraphQLEnumType({
      values: sequelizeType.values.reduce(function (obj, value) {
        if (specialChars.test(value)) {
          value = value.split(specialChars).reduce(function (reduced, val, idx) {
            var newVal = val;
            if (idx > 0) {
